...
import EpicDetailsModal from "@/app/(pages)/(layout)/project/[projectId]/epics/EpicDetails";
import { ProjectEpic, ProjectTask } from "@/Interfaces/AuthInterfaces";
import { useState } from "react";

export default function EpicCard({ epics, projectId }: { epics: ProjectEpic[]; projectId: string }) {

  const [selectedEpic, setSelectedEpic] = useState<ProjectEpic | null>(null);
  const [fetchedTasks, setFetchedTasks] = useState<ProjectTask[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoadingTasks, setIsLoadingTasks] = useState(false);

  if (!epics || !Array.isArray(epics) || epics.length === 0) {
    return <div className="text-center py-12 text-gray-500 font-semibold">No epics found.</div>;
  }
  const handleOpenEpicDetails = async (epic: ProjectEpic) => {
    ...
    try {
      ...
      const data = await displayTasksInsideEpic(epic.id);
      setFetchedTasks(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error("Failed to fetch tasks for this epic", error);
    } finally {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-1">
        {epics.map((epic: ProjectEpic) => (
          <div 
            key={epic.id} 
            onClick={() => handleOpenEpicDetails(epic)}
            className="p-5 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-shadow border-s-4 border-s-[#004E32] cursor-pointer"
          >
        
            <span className='bg-[#82F9BE] px-3 py-1 rounded text-xs font-bold text-[#004E32]'>
              {epic.epic_id}
            </span>
            ...
            <div className='flex mt-4 items-center'>
              <div className='bg-[#65DCA4] text-black p-2 rounded-xl font-semibold w-10 h-10 flex items-center justify-center uppercase text-sm'>
            ...
            <div className='flex justify-between items-center pt-4 mt-6 border-t border-t-[#F1F3FF]'>
              ...
              {epic.created_at && (
                <div className="text-[10px] text-gray-400 tracking-wider uppercase font-semibold">
                  {new Date(epic.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </div>
              )}
            </div>
          </div>
