        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {epics.map((epic: any) => (
            <div key={epic.id} className="p-5 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
              <span className='bg-[#82F9BE] px-3 py-1 '>{epic.epic_id ? epic.epic_id : `EPIC-${epic.id.toString().slice(0, 2)}`}</span>
              <h3 className="font-semibold text-blue-900 text-lg my-4">{epic.title}</h3>
              <p className="text-gray-500 text-sm leading-relaxed">
               ...
               {epic.assignee_id && (
                <div className="text-[10px] text-gray-400 mt-4 tracking-wider uppercase">
                  Created by: {(epic.assignee_id).slice(0.1)}
                </div>
              )}
              {epic.created_at && (
                <div className="text-[10px] text-gray-400 mt-4 tracking-wider uppercase">
